...
# Range of kp and ki values for grid search
kp_values = np.linspace(0.1, 5, 10)  # Adjust range as necessary
ki_values = np.linspace(0.1, 1.0, 10)  # Adjust range as necessary
window_size = 50
...
def main():
    print(f"Using constants: frequency={frequency}, window_size={window_size}")
    ...
    # Grid search over kp and ki values
    for kp in kp_values:
        for ki in ki_values:
            print(f"Evaluating kp={kp}, ki={ki}")
            df_processed, _, _, _ = process_imu_data(df, kp, ki, frequency, window_size, angle_offset)
            avg_error, rmse, pearson_corr = compute_error_metrics(df_processed, start_time, end_time)
            
            if rmse < best_rmse:  # Find the combination with the smallest RMSE
                best_kp = kp
                best_ki = ki
                best_rmse = rmse
                print(f"New best RMSE: {best_rmse:.4f} for kp={best_kp}, ki={best_ki}")
    
    print(f"Best kp: {best_kp}, Best ki: {best_ki}, Best RMSE: {best_rmse:.4f}")
    
    # Re-process with the best kp and ki values
    df_processed, imu0_pitch, imu1_pitch, dorsiflexion_angles = process_imu_data(df, best_kp, best_ki, frequency, window_size, angle_offset)
    
    # Plot angles with best parameters
    plot_angles(df_processed, time_offset, start_time, end_time)
    
    # Compute final error metrics with best parameters
    avg_error, rmse, pearson_corr = compute_error_metrics(df_processed, start_time, end_time)
