...
# Constants
csv_file = "/Users/francescobalanzoni/Documents/Python/MEng/3YP/mobisense/software-group/data-working/assets/mar12exp/SyncedData_OpenCapAngle_IMU/limping3_synced.csv"
frequency = 100
start_time = 0.5
end_time = 11
angle_offset = -5
...
alpha = 0.1  # Complementary filter blending factor
window_size = 50
...
def process_imu_data(df, alpha, frequency, window_size, angle_offset):
    complementary = Complementary(gain=alpha)
    ...
    for i in range(len(df)):
        ...
        q0 = complementary.update(q0, gyr=gyro0, acc=accel0)  # Update quaternion
        r0 = R.from_quat(q0)
        ...
        q1 = complementary.update(q1, gyr=gyro1, acc=accel1)  # Update quaternion
        r1 = R.from_quat(q1)
...
def plot_angles(df, time_offset, start_time, end_time):
    ...
    plt.plot(df_filtered['time'], df_filtered['dorsiflexion_angle'], label='Complementary estimate', color='red', linestyle='--')
...
def main():
    print(f"Using constants: alpha={alpha}, frequency={frequency}, window_size={window_size}")
    df = load_imu_data(csv_file)
    df, imu0_pitch, imu1_pitch, dorsiflexion_angles = process_imu_data(df, alpha, frequency, window_size, angle_offset)
